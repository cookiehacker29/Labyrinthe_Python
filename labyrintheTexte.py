...
def saisirOrdre(lmt):
    ...
    """
    x=input("Si vous souhaiter tourner la carte envoyer T sinon si vous souhaiter insérer à une carte envoyer le la direction de l'insertion ('NESO')")
    if x=='T':
        return x,x
    y=int(input("Choisissez la rangée ou vous souhaiter décaler les cartes"))
    if str(y) in '135':
        return x,y
    else:
        return -1,-1
...
def saisirDeplacement(lmt):
    ...
    if x>=0 and x<=7 and y>=0 and x<=7:
        return x,y
    else:
        return -1,-1
